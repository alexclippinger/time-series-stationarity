...
stationarized_data <- function() {
  ...
  solution <- selectInput("solution", "Solution for Stationarizing:",
                          choices = c("First Difference", "De-trending", "Log Transformation", "De-seasonalizing"))
  ...
  shinyApp(
    ...
    ui <- fluidPage(
      tags$h3("Testing Stationarizing Methods"),
      tabPanel(
        "Time Series",
    ...
    server <- function(input, output, session) {
      ...
      data <- eventReactive(c(input$regenerate, input$stationarity, input$nonstationary_type), ignoreNULL = FALSE, {
        ...
        if (input$nonstationary_type == "Unit Root") {
        ...
        } else if (input$nonstationary_type == "Trend") {
          trend <- seq_along(1:n)
        ...
        } else if (input$nonstationary_type == "Varying Variance") {
          ts_data <- data.frame(
            date = seq_date,
      ...
      standardized_data <- reactive({
        ...
        if (input$solution == "First Difference") {
          st_data <- data() %>% 
            mutate(value = value - dplyr::lag(value))
        } else if (input$solution == "De-trending") {
          # Fit a linear model to find the trend
          lm <- lm(value ~ date, data = data())
      ...
      output$standardized_plot <- renderPlotly({
        p <- ggplot(standardized_data(), aes(x = date, y = value)) +
          geom_line() +
          geom_point(size = 0.5) +
          labs(x = "Date", y = "Value",
               title = paste("Standardized Time-Series")) +
          theme_minimal()
